...
from models import Base, Comments, CommentsOrm, DocsNotes, MessId, Message, MessageOrm, NewUser, TaskEdit, Tasks, TasksOrm, User, UserFio, UserInfo, UserOrm, Docs, DocsOrm 
...
async def messages(id: int, session: SessionDep, current_user: UserInfo = Depends(get_current_user)):
    if current_user.userid > 0:
        ...
        return {"result": "the user is not authorized"}
...
async def add_user(new_user: Annotated[NewUser, Form()], session: SessionDep):
    if (new_user.password1 == new_user.password2):
        ...
        response = RedirectResponse(url="/", status_code=303)
        #response.set_cookie(key="flash_msg", value= ERROR_MESSAGES_EN.get("password_mismatch"] if settings.language == "en" else ERROR_MESSAGES_RU.get("password_mismatch"], httponly=True)
...
async def message_check_read(mess_read: MessId, session: SessionDep, current_user: UserInfo = Depends(get_current_user)):
    if (current_user.userid > 0) and (mess_read.username != current_user.username) and (mess_read.id > 0):
        sql = text("""
...
async def message_like(like: MessId, session: SessionDep, current_user: UserInfo = Depends(get_current_user)):
    if (current_user.userid > 0) and (like.username != current_user.username) and (like.id > 0):
        sql = text("""
...
async def del_message(id: int, session: SessionDep, current_user: UserInfo = Depends(get_current_user)):
    if (current_user.userid > 0) and (id > 0):
...
@app.post("/comments/add",  tags=["Communicator", "new message"], summary="Add a new message")
